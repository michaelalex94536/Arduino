...
float phi;   // Tilt in "Y" direction - same as roll
...
void loop() {
  ...
  theta = -atan2(acc.x(), acc.z()) * (180.0/pi); // Need minus sign so nose up is +ve angle
  phi = -atan2(acc.y(), acc.z()) * (180.0/pi);   // Need minus sign so left side up is +ve angle
